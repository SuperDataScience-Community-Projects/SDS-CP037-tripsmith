...
preference_options = [
    "Museums", "Restaurants", "Outdoors", "Culture", "Shopping",
    "Nightlife", "Historic Sites", "Beaches", "Hiking", "Parks"
]
...
with st.sidebar:
    ...
    hotel_budget = st.number_input("Hotel Budget per Night (required)", min_value=1, value=150, step=10)
    activity_budget = st.number_input("Activities Budget (optional)", min_value=0, value=300, step=25)

    prefs = st.multiselect("Preferences", options=preference_options, default=["Museums", "Restaurants", "Outdoors"]) 
...
if submitted:
    ...
    with col_main:
        ...
        cA, cB = st.columns(2)
        with cA:
            st.metric("Hotel (per night)", f"${hotel_budget:,.0f}")
        with cB:
            st.metric("Activities", f"${(b.get('activity_budget') or 0):,.0f}")
